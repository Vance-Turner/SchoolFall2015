...
% First, we get the function to be integrated.
    ...
    % convergence.
    innerTestCount = 100;
    % We continue to do MC until we achieve desired accuracy!
    while abs(MCintegral-MLBintegral) > accuracy
    ...
    disp('Now calcualting order of convergence...');
    disp(sprintf('a=%d',log(MCintegral)/log(MCintegral-oldIntegral)));
end
